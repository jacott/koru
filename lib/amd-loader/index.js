...
const wrap = (code) => wrapCode + code + '})()';

const SCRIPT_OPTIONS = {displayErrors: true, timeout: 5000};

Context.prototype.loadModule = function (mod) {
  const oldCtx = Module.currentCtx;
  Module.currentCtx = this;
  try {
    wrapCode ??= "(() => {'use strict';const {define, nodeRequire: require, requirejs: " +
      globalName + '} = __requirejsVars__; ';
    const script = new vm.Script(wrap(this.readFileSync(mod.uri)), {
      filename: mod.uri, collumnOffset: wrapCode.length});

    script.runInThisContext(SCRIPT_OPTIONS);
...
Context.setGlobalName = (value) => {globalName = value; wrapCode = undefined};
