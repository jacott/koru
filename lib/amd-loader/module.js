...
const define = Module.define = (name, deps, body) => {
  ...
  if (deps == null && typeof body === 'function' && body.length) {
    ...
    body.toString().replace(commentRe, '').replace(requireRe, (match, quote, dep) => {
      deps.push(dep);
    });
  }
  if (typeof name === 'string') {
...
const runBody = (mod) => {
  let result;
  if (mod.requireDeps) {
    const args = mod.requireDeps;
    mod.requireDeps = null;
  ...
  const require = (id, callback, error) => {
    if (Array.isArray(id)) {
      ...
      ids.forEach((id) => {
        ...
        switch (id) {
          case 'require':
            results[id] = mod.require;
            return;
          case 'exports':
            results[id] = mod.exports;
            return;
          case 'module':
            results[id] = mod;
            return;
    ...
    for (const pId in waiting) {
      ...
      for (const id in mods) {
        ...
        if (mod !== undefined) {
          ...
          if (curr.callback) {
            if (mod.callback) {
              curr.callback.forEach((arg) => {
                mod.callback.push(arg);
              });
            }
            if (mod.error) {
              curr.error.forEach((arg) => {
                mod.error.push(arg);
              });
            }
          }
