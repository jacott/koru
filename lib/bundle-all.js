...
var UglifyJS = require("../node_modules/yaajs/node_modules/uglify-js");
  ...
  Fiber(function () {
    try {
      ...
      compiler.compile(optConfig, function (codeTree) {
        var code = ["const isServer = false, isClient = true;", fs.readFileSync(Path.join(__dirname, '../node_modules/yaajs/yaa.js')), cfgStr];
        for (;codeTree; codeTree = codeTree.next) {
          code.push(codeTree.code);
        }
        code = code.join("\n");
        if (optConfig.uglify) {
          cfg.extend(optConfig.uglify, {
            fromString: true,
          });
          code = UglifyJS.minify(code, optConfig.uglify).code.replace(/^[^;]*;/,'');
        }
