...
function findTop() {
  ...
  throw new Error("You are no in a koru application directory!");
}
...
function template(inFilename, outFilename, opts) {
  ...
  fs.writeFileSync(outFilename, data.map((text, i) => {
...
module.exports = {
  koruTop, topDir, template,
  mkdir_p, loadEnv,
  pathExists, updateFile, system,
  fileizeString, classifyString, camelizeString,
  findTop, skelDir,
};
