  ...
  TH.testCase(module, ({before, after, beforeEach, afterEach, group, test}) => {
    ...
    group('connect', () => {
      test('bad options', async () => {
        ...
        await assert.exception(() => new PgConn(PgType).connect({port: 5432, user: 'bad-user'}), {
          severity: 'FATAL',
          code: '28000',
        });
      ...
      group('connection string parsing', () => {
        let socket, connect;
        beforeEach(() => {
          socket = {write: stub(), once: stub(), on: stub(), removeListener: stub()};
        ...
        test('bad connection string', async () => {
          const assertError = (str, message = 'Invalid connection string') =>
            assert.exception(() => new PgConn(PgType).connect(str), {severity: 'FATAL', message});
        ...
        test('error during connect', async () => {
          ...
          assert.calledWith(
            socket.write,
            new Uint8Array([0, 0, 0, 16, 0, 3, 0, 0, 117, 115, 101, 114, 0, 120, 0, 0]),
          );
        ...
        test('unix url', () => {
          const p = new PgConn(PgType).connect(
            `postgresql:///mydb?user=myuser&application_name=myapp&client_encoding=UTF8`,
          );
        ...
        test('network url', () => {
          const p = new PgConn(PgType).connect(
            `postgresql://me:pw@test.com/mydb?application_name=myapp&keepalives=1`,
          );
          ...
          assert.equals(pgproto.options, {user: 'me', database: 'mydb', application_name: 'myapp'});
        });
      });
    });
    ...
    test('handle bad message during connect', async () => {
      ...
      await assert.exception(() => p, {severity: 'FATAL', message: 'test failure'});
    });
    ...
    group('with connection', () => {
      ...
      test('bad query', async () => {
        await assert.exception(() => client.exec(`SELECT $1`), {
          severity: 'ERROR',
      ...
      test('tags', async () => {
        ...
        assert.equals(
          await client.exec(`create table "Test1" (_id TEXT PRIMARY KEY, v int4)`),
          'CREATE TABLE',
        );
        ...
        assert.equals(await client.exec(`SELECT * from "Test1"`), [{_id: 'hello'}, {
          _id: 'world',
          v: 123,
        }]);
        assert.equals(
        ...
        assert.equals(
          await client.exec(`declare cl1 cursor FOR SELECT _id FROM "Test1"`),
          'DECLARE CURSOR',
        );
      ...
      test('bytea', async () => {
        ...
        assert.equals(await client.exec(`SELECT $1::bytea as b`, ['\\x0001027fff'], [25]), [{
          b: u8,
        }]);
      ...
      test('use after error', async () => {
        await assert.exception(() => client.exec(`SELECT $1 $2`, [1, 2]), {code: '42601'});
        assert.equals(await client.exec('SELECT 1 as a'), [{a: 1}]);
      });

      test('exec with params', async () => {
        after(() => {
          client.formatOptions = {};
        });
        ...
        assert.equals(await client.exec(`SELECT $1::int4 as n`, ['123'], [0], [1]), [{n: 123}]);

        assert.equals(await client.exec(`SELECT $1 as n`, ['123'], [23], [1]), [{n: 123}]);
      });
    });
  });
});
