define((require, exports, module) => {
  ...
  const prepareNewVersion = (hash = window.KORU_APP_VERSION) => {
    ...
    let resolve;
    prepareNewVersionPromise = new Promise((r) => {
      resolve = r;
    });
  ...
  const SWManager = makeSubject(
    {
      start() {
        ...
        if (
          serviceWorker.controller !== null && !serviceWorker.controller.scriptURL.endsWith(SW_NAME)
        ) {
      ...
      onUpdateWaiting(callback) {
        ...
        if (
          registration !== null && registration.installing == null && registration.waiting != null
        ) {
