define((require, exports, module) => {
  ...
  TH.testCase(module, ({after, beforeEach, afterEach, group, test}) => {
    ...
    group('validation', () => {
      test('simple', () => {
        assert(RichText.isValid('some\nlines', [BOLD, 0, 2, 3]));
        assert(RichText.isValid(''));
        assert(RichText.isValid());
        assert(RichText.isValid('some\nlines'));
        refute(RichText.isValid('some\nlines', []));
        refute(RichText.isValid('some\nlines', [BOLD, 0, 2, 1]));
        refute(RichText.isValid('some\nlines', [BOLD, -1, 2, 3]));
        refute(RichText.isValid('some\nlines', [BOLD, 1]));
        refute(RichText.isValid('some\nlines', [-1, 1]));
        refute(RichText.isValid('some\nlines', 'hello'));
        refute(RichText.isValid('345', NaN));
        refute(RichText.isValid([1, 2]));
        assert(
          RichText.isValid('code:rb\n"a"\ncode:rb\nb', [CODE, 0, 1, 48, OL, 0, 3, CODE, 1, 1]),
        );
      });
    });

    test('fontType', () => {
      RichText.mapFontNames({cursive: 'foo-face'});
      assert.same(RichText.fontType(0), 'sans-serif');
      assert.same(RichText.fontType('5'), 'cursive');
      assert.same(RichText.fontType(5), 'cursive');
      assert.same(RichText.fontType('cursive'), 'cursive');
      assert.same(RichText.fontType('foo-face'), 'cursive');
      assert.same(RichText.fontType('serif'), 'serif');
      assert.same(RichText.fontType(''), 'sans-serif');
    });

    test('just text', () => {
      const doc = 'Hello world';

      const html = RichText.toHtml(doc, null, para);

      assert.same(html.outerHTML, '<p><div>Hello world</div></p>');

      assert.equals(RichText.fromHtml(html), [doc, null]);
    });

    test('fragment', () => {
      const rt = RichText.fromHtml(Dom.h([{b: 'foo'}, ' bar']));
    ...
    test('div with multi-line text', () => {
      ...
      const html = RichText.toHtml(doc);
      ...
      assert.equals(RichText.fromHtml(arround), [doc, null]);
    });

    test('empty li', () => {
      const doc = '\n\n', markup = [OL, 0, 1, LI, 0, 0, LI, 1, 0];

      const html = RichText.toHtml(doc, markup, para);
      assert.same(html.outerHTML, '<p><ol><li><br></li><li><br></li></ol><div><br></div></p>');

      assert.equals(RichText.fromHtml(para), [doc, markup]);
    });

    test('simple', () => {
      const doc = 'a\nb';
      const html = document.createElement('p');
      html.appendChild(RichText.toHtml(doc));

      assert.same(html.outerHTML, '<p><div>a</div><div>b</div></p>');

      assert.equals(RichText.fromHtml(html), ['a\nb', null]);
    });

    test('nested', () => {
      const doc = 'brave\nnew\nworld\nnow',
        markup = [NEST, 0, 2, NEST, 0, 0, NEST, 2, 0, NEST, 1, 0];
      const html = RichText.toHtml(doc, markup, para);
      assert.same(
        html.outerHTML,
        '<p><blockquote>' +
    ...
    test('headers', () => {
      const json = {
        ol: {
          li: [{h1: 'heading 1'}, {h5: ['heading ', {style: 'font-style: italic;', span: '2'}]}, {
            div: [{style: 'font-weight: bold;', span: 'the'}, ' para'],
          }],
        },
      };

      const mu = RichText.fromHtml(Dom.h(json));

      const html = RichText.toHtml(...mu);
    ...
    test('strike-through', () => {
      ...
      assert.equals(Dom.htmlToJson(html), [{
        div: ['a', {style: 'text-decoration: line-through;', span: 'b'}, 'c', {
    ...
    test('inline styles', () => {
      const doc = 'brave\nnew\nworld',
        markup = [BOLD, 0, 0, 5, ITALIC, 1, 0, 2, BOLD, 0, 2, 3, UNDERLINE, 1, 3, 4];
      const html = RichText.toHtml(doc, markup, para);
      assert.same(
        html.outerHTML,
        '<p><div><span style="font-weight: bold;">brave</span></div><div>' +
    ...
    test('list and nesting', () => {
      const doc = 'It´s a\nbrave\n new world now',
        markup = [NEST, 1, 1, BOLD, 0, 0, 5, ITALIC, 1, 5, 10];

      const html = RichText.toHtml(doc, markup, para);

      assert.same(
        html.outerHTML,
        '<p><div>It´s a</div><blockquote><div><span style="font-weight: bold;">brave</span></div><div> new <span style="font-style: italic;">world</span> now</div></blockquote></p>',
      );

      assert.equals(RichText.fromHtml(html), [doc, markup]);
    });

    test('complex', () => {
      const complex =
        '<div>he</div><div>-llo world<span style="font-weight: bold;">in <span style="font-style: italic;">here</span>' +
      ...
      assert.equals(RichText.toHtml(doc, markup, document.createElement('div')).innerHTML, complex);
      assert.equals(RichText.fromHtml(html), [doc, markup]);
    });

    test('special', () => {
      RichText.registerLinkType({
        id: 1,
        class: 'foo',
        fromHtml(node) {
          return node.getAttribute('href').replace(/_foo$/, '');
        },
        toHtml(node, ref) {
      ...
      TH.onEnd(() => {
        RichText.deregisterLinkType(1);
      });
      const html = Dom.h({div: {a: 'a foo', class: 'foo', $href: 'link_to_foo'}});
      assert.equals(RichText.fromHtml(html), ['a foo (link_to)', [LINK, 0, 0, 15, 1, 5]]);
      assertConvert(TH.normHTMLStr(html.outerHTML));
    });

    test('linkType', () => {
      assert.equals(RichText.linkType(0), {
        id: 0,
        class: '',
        fromHtml: TH.match.func,
        toHtml: TH.match.func,
      });
    });
    ...
    test('skips empty text', () => {
      const html = Dom.h({
        p: {ol: [{li: 'one'}, {li: ['two', {br: ''}, document.createTextNode('')]}]},
      });
    ...
    test('font', () => {
      RichText.mapFontNames({cursive: "'foo Face', foo-face"});

      assert.equals(RichText.fontType("'foo Face', foo-face"), 'cursive');
      assert.equals(RichText.fontType('foo Face, foo-face'), 'cursive');

      assertBothConvert(
        '<div>one<font color="#ff0000" face="foo-face" size="4">two</font>three</div>',
      ...
      assert.equals(RichText.fromHtml(Dom.h({div: {font: 'foo', $face: 'foo-face'}})), ['foo', [
        FONT,
        0,
        0,
        3,
      ...
      assertBothConvert(
        '<div>one<font color="#ff0000">two</font>three</div>',
        '<div>one<span style="color: rgb(255, 0, 0);">two</span>three</div>',
      );
      assertBothConvert(
        '<div>one<font face="initial">two</font>three</div>',
        '<div>one<span style="font-family: initial;">two</span>three</div>',
      );
      assertBothConvert(
        '<div>one<font face="myface">two</font>three</div>',
        '<div>one<span style="font-family: myface;">two</span>three</div>',
      );
      assertBothConvert(
        '<div>one<font face="serif">two</font>three</div>',
      ...
      assertBothConvert(
        '<div>one<font face="sans-serif">two</font>three</div>',
        '<div>one<span style="font-family: sans-serif;">two</span>three</div>',
      );
      assertBothConvert(
        '<div><font face="foo-face">two</font></div>',
        `<div><span style="font-family: 'foo Face', foo-face;">two</span></div>`,
      );
    });

    test('alignment', () => {
      assertConvert('<div align="right">foo</div>', '<div style="text-align: right;">foo</div>');
      assertBothConvert(
        '<div style="text-align: right;"><ol><li>a</li><li>b</li></ol></div>',
        '<ol><li><div style="text-align: right;">a</div></li><li><div style="text-align: right;">b</div></li></ol>',
      );
      assertBothConvert(
        '<div style="text-align: center;"><ol><li>foo</li></ol></div>',
        '<ol><li><div style="text-align: center;">foo</div></li></ol>',
      );
      assertBothConvert(
        '<div><ol style="text-align: justify;"><li>abc</li><li><br></li></ol></div>',
        '<ol><li><div style="text-align: justify;">abc</div></li><li><div style="text-align: justify;"><br></div></li></ol>',
      );
      assertConvert(
        '<div style="text-align: left;"><ol><li>abc</li></ol></div>',
        '<ol><li><div style="text-align: left;">abc</div></li></ol>',
      );
      assertConvert(
        '<div style="text-align: justify;">hello</div><div style="text-align: left;">world</div>',
      );
      assertConvert('<blockquote><div style="text-align: right;">one</div></blockquote>');
      assertBothConvert(
        '<blockquote><div style="text-align:center;">one<br>two</div></blockquote>',
    ...
    test('hiliteColor', () => {
      assertConvert(
        '<div>one<span style="background-color:#ffff00">two</span>three</div>',
        '<div>one<span style="background-color: rgb(255, 255, 0);">two</span>three</div>',
      );
    });

    test('includeTop', () => {
      const rt = RichText.fromHtml(Dom.h({ol: {li: 'line'}}), {includeTop: true});
      assert.equals(rt, ['line', [OL, 0, 0, LI, 0, 0]]);
    });

    test('code', () => {
      assertBothConvert(
        '<ol><li><div>one</div><pre data-lang="text"><div>foo</div></pre></li></ol>',
      ...
      p.innerHTML =
        '<div><div><pre>One</pre><pre>Two</pre></div><div><pre data-lang="text"></pre></div></div>';
      const rt = RichText.fromHtml(p);
      ...
      assertConvert(
        '<pre data-lang="ruby"><span class="k">Class</span> <span class="no">Abc</span><br>' +
      ...
      assertConvert(
        '<pre data-lang="ruby"><span class="k">Class</span> <span class="no">Abc</span></pre>',
      );
      assertConvert(
        '<pre data-lang="text">stuff <span class="nd">var</span><br><br>foo = <span class="s2">_bzr_</span>;<br></pre>',
      );
      assertConvert(
      ...
      assertBothConvert(
        '<div><pre><ol><li>hello</li><li>wo<b>rl</b>d</li></ol></pre></div>',
        '<pre data-lang="text">hello<br>world</pre>',
      );
      assertBothConvert(
        '<div><pre>hello<div><br></div>new<br>world<div>now</div></pre></div>',
        '<pre data-lang="text">hello<br><br>new<br>world<br>now</pre>',
      );
      assertBothConvert(
        '<div>Some <code>code in</code> here</div><pre data-lang="javascript">one\ntwo\nthree</pre>',
    ...
    test('font-weight', () => {
      ...
      assertConvert('<span style="font-weight:600">x</span>', '<div>x</div>');
      assertConvert(
    ...
    test('multiple', () => {
      ...
      assertConvert(
        '<div><span style="font-weight:normal;font-style:normal;text-decoration:line-through;">text</span></div>',
      ...
      assertBothConvert(
        '<ol><li>one</li></ol><span><ul><li><span>two</span><br></li></ul></span><ol><li>three</li></ol>',
      ...
      assertConvert(
        '<ol><li>hey</li></ol><span>now</span><br><div><span><br></span></div>',
      ...
      assertConvert(
        '<blockquote><ul><li>one</li><ol><li>2.1</li><li>2.2</li></ol><li>TH<b><i>R</i></b>EE 3</li></ul></blockquote>',
      ...
      assertConvert(
        '<section><section>sec<span>ti</span>on</section></section>',
        '<div>section</div>',
      );
      assertBothConvert(
        '<ol><li><b>hey</b></li><ol><li><b>now</b></li></ol></ol>',
        '<ol><li><span style="font-weight: bold;">hey</span></li><ol><li>' +
          '<span style="font-weight: bold;">now</span></li></ol></ol>',
      );
      assertBothConvert(
        '<div><b>Hello </b></div><div><b><i><br></i></b></div><div><b><i>dffd</i></b></div>' +
  ...
  const assertConvert = (text, expect = text) => {
    ...
    assert.elide(() => {
      assert.msg(() => rt).equals(
        TH.normHTMLStr(RichText.toHtml(rt[0], rt[1], document.createElement('p')).innerHTML)
          .replace(/\&quot;/g, "'"),
        expect,
      );
    });
  ...
  const inspectFrag = (frag) => {
    const result = [];
    util.forEach(frag.childNodes, (elm) => {
