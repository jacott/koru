isServer && define((require, exports, module) => {
  ...
  TH.testCase(module, ({after, beforeEach, afterEach, group, test}) => {
    ...
    group('Commander', () => {
      ...
      const innerSubject = () =>
        api.innerSubject(Migration[isTest].Commander, null, {
          abstract() {
            /**
      ...
      test('createTable', async () => {
        ...
        const migBody =
          //[
          define(() => (mig) => {
            mig.createTable('Label', {
              name: 'text',
              backgroundColor: {type: 'color', default: '#ffffff'},
            }, [{columns: ['name DESC', '_id'], unique: true}, ['backgroundColor']]); //  columns
          });
        //]
        ...
        const indexes = await client.query(
          'select * from pg_indexes where tablename = $1 ORDER BY indexname',
          ['Label'],
        );
      ...
      test('field strings for create', async () => {
        after(() => {
          delete ModelMap.TestTable;
        });
        await sut.addMigration('20151003T20-30-20-create-TestModel', (mig) => {
          spy(sut._client, 'query');
          mig.createTable({name: 'TestTable', fields: ['myName', "age:jsonb default '7'::jsonb"]});
        });
        assert.calledWith(
          sut._client.query,
          `CREATE TABLE "TestTable" (_id text collate "C" PRIMARY KEY,"myName" text,"age" jsonb default '7'::jsonb)`,
        );
      });

      test('explict primary key in create', async () => {
        await sut.addMigration('20151003T20-30-20-create-TestModel', (mig) => {
          mig.createTable('TestTable', {name: 'text', foo: {type: 'integer primary KEY'}});
        });
      ...
      test('no primary key', async () => {
        await sut.addMigration('20151003T20-30-20-create-TestModel', (mig) => {
          mig.createTable({
            name: 'TestTable',
            primaryKey: false,
            fields: {name: 'text', foo: {type: 'integer'}},
          });
        });
        await client.query('INSERT INTO "TestTable" (foo, name) values ($1,$2)', [2, 'foo']);
        await client.query('INSERT INTO "TestTable" (foo, name) values ($1,$2)', [2, 'foo']);
      ...
      test('composite primary key', async () => {
        await sut.addMigration('20151003T20-30-20-create-TestModel', (mig) => {
          mig.createTable({
            name: 'TestTable',
            primaryKey: ['foo', 'Bar'],
            fields: {name: 'text', foo: {type: 'integer'}, Bar: {type: 'text'}},
          });
        });
        await client.query('INSERT INTO "TestTable" (foo, "Bar", name) values ($1,$2,$3)', [
          2,
          'bar1',
          'foo',
        ]);
        ...
        const doc = (await client.query('SELECT * from "TestTable"'))[0];
        assert.same(doc.foo, 2);
        assert.same(doc.Bar, 'bar1');
        refute.hasOwn(doc, '_id');
        await assert.exception(
          () =>
            client.query('INSERT INTO "TestTable" (foo, "Bar", name) values ($1,$2,$3)', [
              2,
              'bar1',
              'foo2',
            ]),
      ...
      test('unlogged create', async () => {
        ...
        await sut.addMigration('20151003T20-30-20-create-TestModel', (mig) => {
          mig.createTable({
            name: 'TestTable',
            unlogged: true,
            fields: {name: 'text', foo: {type: 'integer primary KEY'}},
          });
        });
        assert.calledWith(
          client.query,
      ...
      test('addIndex', async () => {
        ...
        let index =
          (await client.query('select indexdef from pg_indexes where indexname = $1', [
            'Book_title__id',
          ]))[0];
        ...
        assert.equals(
          (await client.query('select indexname from pg_indexes where tablename = $1', ['Book']))
            .map((r) => r.indexname),
          ['Book_pkey'],
        );
      });
      ...
      group('addColumns', () => {
        ...
        test('addColumns by object', async () => {
          ...
          const migBody =
            //[
            define(() => (mig) => {
              mig.addColumns('Book', {pageCount: {type: 'int8', default: 0}, author_id: 'id'});
            });
          //]
    ...
    test('migrateTo', async () => {
      ...
      assert.same(
        (await client.query('select exists(select 1 from pg_catalog.pg_class where relname = $1)', [
          'TestTable',
        ]))[0].exists,
        false,
      );
    ...
    test('recordAllMigrations', async () => {
      ...
      assert.equals(
        (await client.query('SELECT name from "Migration" order by name')).map((d) => d.name),
        [
