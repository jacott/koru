  ...
  TH.testCase(module, ({after, beforeEach, afterEach, group, test}) => {
    ...
    test('config', () => {
      assert.equals(v.km.map, {
        X: ['foo', v.foo],
        Q: {'*\u0003': {2: ['mbar2', v.mbar2]}, X: {1: ['bar', v.bar], 2: ['bar2', v.bar2]}},
        '*\u0002': {A: ['foo2', v.foo2]},
        '*\u0004': {B: ['foo3', v.foo3]},
    ...
    test('mapCtrlToMeta', () => {
      const f = () => {};
      const km = sut({f1: [sut.ctrl + 'A', f], f2: [sut.shift + 'B', f]}, {mapCtrlToMeta: true});
      ...
      km.addKeys({f5: [sut.shift + sut.ctrl + 'E', f]});
    ...
    test('multi key', () => {
      ...
      assert.calledOnceWith(
        v.bar,
        TH.match((ev) => ev.type === 'keydown' && ev.which === 49),
        'bar',
      );
