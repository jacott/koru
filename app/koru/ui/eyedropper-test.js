isClient && define((require, exports, module) => {
  ...
  TH.testCase(module, ({before, after, beforeEach, afterEach, group, test}) => {
    ...
    test('pick one option', async () => {
      const div = Dom.h({
        style: 'background-color:rgba(255, 0, 255, 0.5);width:300px;height:200px',
      });
      ...
      const color = await new Promise((resolve, reject) => {
        ...
        TH.trigger(document.body, 'pointerdown', {
          clientX: bbox.left + .5 * bbox.width,
          clientY: bbox.top + .5 * bbox.height,
        });
      });
    ...
    test('pick multi options', async () => {
      const div = Dom.h({
        style: 'background-color:#ffee33;',
        div: {style: 'border: 1px solid #556644;color:rgba(255, 0, 0, .8);', span: ['foo bar']},
      });
      document.body.appendChild(div);
      document.body.appendChild(
        Dom.h({
          style:
            'position:absolute;top:0;background-color:rgba(250,130,200,.09);width:300px;height:200px',
        }),
      );
    ...
    test('getPointColors html', async () => {
      const div = Dom.h({
        style: 'margin:20px;background-color:rgba(100, 2, 1, 0.9);',
        div: {
      ...
      assert.calledWith(sut.getColorFromImage, dd, bbox.left + 1, bbox.top + 1);
      ...
      before(() => {
        const div = Dom.h({
          ...
          svg: {
            ...
            style: 'stroke:#f4a3c2;stroke-width:5;',
          },
        });
        document.body.appendChild(div);
    ...
    test('svg getColorFromImage', async () => {
      const div = Dom.h({
        ...
        svg: {
          ...
          height: 100,
          fill: 'rgba(51, 102, 153, 0.5)',
          style: 'stroke:rgba(151, 22, 253, 0.8);stroke-width:5;',
        },
      });
      document.body.appendChild(div);
      const rect = Dom('rect');
    ...
    test('intercept', async () => {
      document.body.appendChild(
        Dom.h([{
          ...
          id: 'div2',
          style:
            'background-color:rgba(0,0,0,.01);position:absolute;top:0;left:0;width:100px;height:100px',
        }]),
      );

      const colors = await sut.getPointColors(10, 10, {
        intercept: (elm, cs) => {
