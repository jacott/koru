...
function modalizeCallback(event) {
  ...
  if (Koru.getClosestClass(event.target, 'anyModal')) return;
  if (event.type === 'keyup') {
    if (event.which === 27) {
      if (! Koru.parentOf(elm, event.target))
  ...
  } else if (! Koru.parentOf(elm, event.target))
    return modalize.func.call(this, event);
};
...
Tpl.$extend({
  ...
  disableFields: function (form) {
    Koru.forEach(form, Koru.WIDGET_SELECTOR, function (elm) {
  ...
  clearErrors: function (form) {
    ...
    while(msgs.length) {
      Koru.removeClass(msgs[msgs.length - 1], 'error');
    }
  },
...
Tpl.Radio.Button.$helpers({
  checked: function () {
    Koru.setBoolean('checked', this.checked);
  },
});
...
Koru.registerHelpers({
  errorMsg: function () {
    var elm = Koru.current.element;
    return Koru.hasClass(elm, 'errorMsg') ? elm : errorMsg.cloneNode(true);
  },

  checked: function (value) {
    Koru.setBoolean('checked', value);
  },
...
function field(doc, name, options) {
  ...
  case 'markdownEditor':
    return Koru.MarkdownEditor.$autoRender({content: doc[name], options: App.reverseExtend({"data-errorField": name}, options)});
  case 'onOff':
...
function changeColorEvent(formId, field) {
  return function (event) {
    ...
    Koru.ColorPicker.choose(doc[field], function (result) {
      if (result) {
        doc[field] = result;
