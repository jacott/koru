...
Tpl.$extend({
  clear: function (elm) {
    if (! Koru.hasClass(elm, 'mdEditor'))
      elm = elm.parentNode;
    var input = Koru.getCtx(elm).data.inputElm;
    input.setAttribute('contenteditable', 'false');
    input.textContent = '';
    input.setAttribute('contenteditable', 'true');
    Koru.remove(Koru.getCtx(input).selectItem);
    Koru.addClass(elm, 'empty');
  },
  ...
  $created: function (ctx, elm) {
    ...
    data.toolbar = Koru.getCtx(tbElm);
...
Tpl.$events({
  'focusin': function (event) {
    Koru.addClass(this, 'focus');
    updateToolbar($.ctx.data);
  },

  'focusout': function (event) {
    Koru.removeClass(this, 'focus');
  },
});
...
Input.$events({
  ...
  keypress: function (event) {
    ...
    if (ctx.mentionState != null && ctx.mentionState < 3) {
      ...
      tnode.textContent = '@';
      var span = Koru.html({tag: 'span', "class": 'ln', text: ch});
    ...
    case 64:
      if (event.shiftKey) {
        ...
        var span = Koru.html({tag: 'span', "class": 'lm', text: '@'});
  ...
  'input': function (event) {
    ...
    if (! Koru.hasClass(mdEditor, 'empty') === isEmpty)
      Koru.setClass('empty', isEmpty, mdEditor);
  },
  ...
  'paste': function (event) {
    if ('clipboardData' in event) {
      ...
      if (index !== -1) {
        var md = App.Markdown.fromHtml(Koru.html('<div>'+event.clipboardData.getData(items[index].type)+'</div>'));
...
function getTag(tag) {
  ...
  return Koru.searchUpFor(start, function (elm) {
