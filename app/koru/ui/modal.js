define((require) => {
  ...
  const lateKeydownCallback = (event) => {
    switch (event.which) {
    case 9: return;
    case 27:
      Dom.stopEvent(event);
  ...
  const Modal = {
    init: (options) => {
      ...
      if (topModal == null) {
        ...
        document.addEventListener('keydown', lateKeydownCallback);
      }
      ...
      options.ctx.onDestroy(() => {
        ...
        if (options === topModal) {
          ...
          if (topModal == null) {
            ...
            document.removeEventListener('keydown', lateKeydownCallback);
          }
        } else {
          for (let last = topModal, curr = topModal && topModal.prev;
