  ...
  TH.testCase(module, ({before, after, beforeEach, afterEach, group, test}) => {
    ...
    group('with change', () => {
      ...
      test('clone', () => {
        ...
        //[
        change = DocChange.change(new Book({_id: 'book1', title: 'Animal Farm'}), {
          title: 'Fanimal Arm',
        });
      ...
      test('was', () => {
        ...
        //[
        const dc = DocChange.change(new Book({_id: 'book1', title: 'Animal Farm'}), {
          title: 'Fanimal Arm',
        });
        const {was} = dc;
      ...
      test('hasField', () => {
        ...
        //[
        const dc = DocChange.change(new Book({_id: 'book1', title: 'Animal Farm', pages: 112}), {
          title: 'Fanimal Arm',
        });
      ...
      test('hasSomeFields', () => {
        ...
        const dc = DocChange.change(
          new Book({_id: 'book1', title: 'Animal Farm', Author: 'George Orwell', pages: 112}),
      ...
      test('subDocKeys', () => {
        ...
        assert.equals(Array.from(DocChange.change(book, {}).subDocKeys('index')).sort(), [
          'd',
          'h',
          'p',
        ]);
        ...
        assert.equals(
          Array.from(DocChange.change(book, {$partial: {title: null}}).subDocKeys('index')).sort(),
          [],
        );
      });
      ...
      group('subDocs', () => {
        ...
        test('example', () => {
          ...
          for (const dc of change.subDocs('index')) {
            if (dc._id === 'd') {
              ...
              assert.equals(dc.undo, {$partial: {deer: null, donkey: ['$replace', [56, 456]]}});
            } else {
              ++count;
          ...
          assert.equals(ans.map((dc) => dc.doc), [{dog: [123, 234], deer: [34]}, false, {
            horse: [23, 344],
          }, {pig: [3, 34]}]);
        ...
        test('multipart partial', () => {
          const book = new Book({
            _id: 'book1',
            title: 'Animal Farm',
            index: {p: {words: {pig: {occurs: 158}, puppy: {occurs: 2}}}},
          });

          const undo = {$partial: {index: ['p.words.puppy.$partial', ['occurs', 4]]}};
        ...
        test('nested', () => {
          ...
          const undo = {
            $partial: {
              index: ['p.$partial', ['words.$partial', ['puppy', null, 'pig.$partial', [
                'occurs',
                34,
              ]]]],
            },
          ...
          assert.equals(dc.undo, {
            $partial: {words: ['puppy', null, 'pig.$partial', ['occurs', 34]]},
          });

          ans = map(dc.subDocs('words'));
          assert.equals(ans.map((dc) => dc._id), ['pig', 'puppy']);
          assert.equals(ans.map((dc) => dc.type), ['chg', 'add']);
          assert.equals(ans.map((dc) => dc.doc), [{occurs: 158}, {occurs: 2}]);
          assert.equals(ans.map((dc) => dc.undo), [{$partial: {occurs: 34}}, 'del']);
        });
        ...
        test('compond keys', () => {
          const book = new Book({
            _id: 'book1',
            title: 'Animal Farm',
            index: {p: {words: {pig: {syn: 'hog'}, puppy: {syn: 'dog'}}}},
          });

          const undo = {
            $partial: {
              index: ['p.words.pig.syn', 'pork', 'p.words.puppy.syn.$partial', ['$append', 'gy']],
            },
          };
          ...
          assert.equals(dc.undo, {
            $partial: {words: ['pig.syn', 'pork', 'puppy.syn.$partial', ['$append', 'gy']]},
          });

          ans = map(dc.subDocs('words'));
          assert.equals(ans.map((dc) => dc._id), ['pig', 'puppy']);
          assert.equals(ans.map((dc) => dc.type), ['chg', 'chg']);
          assert.equals(ans.map((dc) => dc.doc), [{syn: 'hog'}, {syn: 'dog'}]);
          assert.equals(ans.map((dc) => dc.undo), [{$partial: {syn: 'pork'}}, {
            $partial: {syn: ['$append', 'gy']},
          }]);
        });
      });
