  ...
  TH.testCase(module, ({before, after, beforeEach, afterEach, group, test}) => {
    ...
    test('promisify', async () => {
      ...
      const id = await db.promisify(() =>
        db.transaction(['TestModel'], 'readwrite').objectStore('TestModel').put({
          _id: 'id1',
          name: 'foo',
        })
      );
    ...
    group('queueChange', () => {
      ...
      test('simulated add, update', async () => {
        ...
        after(() => {
          session.state.pendingCount() == 1 && session.state.decPending();
        });
      ...
      test('simulated remove', async () => {
        ...
        await v.db.whenReady();
        {
          ...
          assert.equals(iDoc, {
            _id: 'foo123',
            $sim: [{_id: 'foo123', name: 'foo', age: 5, gender: 'm'}, undefined],
          });
        }
        //]
    ...
    group('loadDoc', () => {
      ...
      test('simulated update', async () => {
        ...
        assert.equals(v.simDocs(), {foo123: [{name: 'foo'}, undefined]});
      });
      ...
      test('simulated remove', async () => {
        ...
        v.db.loadDoc('TestModel', {
          _id: 'foo123',
          $sim: [{_id: 'foo123', name: 'foo2', age: 5, gender: 'f'}, undefined],
        });
        await v.db.whenReady();
      ...
      group('with stopGap$', () => {
        ...
        test('simulated update', async () => {
          ...
          assert.calledWith(
            v.oc,
            DocChange.change(m.is(v.foo123), {name: 'stopGap', gender: 'm'}, undefined),
          );
        });
        ...
        test('non simulated update', async () => {
          ...
          assert.calledWith(
            v.oc,
            DocChange.change(m.is(v.foo123), {name: 'stopGap', gender: 'm'}, 'idbLoad'),
          );
          assert.equals(v.foo123[stopGap$], undefined);
        });
    ...
    test('loadDocs', async () => {
      ...
      //[
      let called = false;
      TestModel.onChange((dc) => {
        db.queueChange(dc);
        called = true;
      });
      const recs = [{_id: 'foo123', name: 'foo', age: 5, gender: 'm'}, {
        _id: 'foo456',
        name: 'bar',
        age: 10,
        gender: 'f',
    ...
    test('getAll', async () => {
      ...
      assert.equals(docs, [{_id: 'foo123', name: 'foo', age: 5, gender: 'm'}, {
        _id: 'foo124',
        name: 'foo2',
        age: 10,
        gender: 'f',
    ...
    group('with data', () => {
      beforeEach(() => {
        v.db = new QueryIDB({
          ...
          version: 2,
          upgrade({db}) {
            db.createObjectStore('TestModel').createIndex('name', 'name', {unique: false});
          },
        });
        v.db.whenReady().then(() => {
          v.foo = v.idb._dbs.foo;
      ...
      test('transaction', async () => {
        ...
        await v.db.whenReady();
        const t = v.db.transaction(
          'TestModel',
          'readwrite',
          v.opts = {oncomplete: stub(), onabort: stub()},
        );
      ...
      test('count', async () => {
        ...
        await v.db.whenReady();
        v.db.count('TestModel', IDBKeyRange.bound('r1', 'r4', false, true)).then((ans) =>
          v.ans = ans
        );
      ...
      test('Index', async () => {
        ...
        await v.db.whenReady();
        v.db.index('TestModel', 'name').getAll(IDBKeyRange.bound('Lucy', 'Ronald', false, true))
          .then((docs) => v.ans = docs);

        v.db.index('TestModel', 'name').getAllKeys(IDBKeyRange.bound('Lucy', 'Ronald', false, true))
          .then((docs) => v.ansKeys = docs);

        await v.db.whenIdle();
        assert.equals(v.ans, [v.r4]);
        assert.equals(v.ansKeys, ['r4']);

        v.db.index('TestModel', 'name').getAll().then((docs) => v.ans = docs);

        v.db.index('TestModel', 'name').getAllKeys().then((docs) => v.ansKeys = docs);

        await v.db.whenIdle();
        assert.equals(v.ans, [v.r3, v.r4, v.r1, v.r2]);
        assert.equals(v.ansKeys, ['r3', 'r4', 'r1', 'r2']);

        v.db.index('TestModel', 'name').count(IDBKeyRange.bound('Lucy', 'Ronald', false, false))
          .then((ans) => v.ans = ans);

        await v.db.whenIdle();
        assert.equals(v.ans, 3);

        v.db.index('TestModel', 'name').get('Ronald').then((docs) => v.ans = docs);
      ...
      test('index cursor', async () => {
        ...
        await v.db.whenReady();
        v.ans = [];
        v.db.index('TestModel', 'name').cursor(null, 'prev', (cursor) => {
          if (!cursor) return;
          v.ans.push(cursor.value);
          cursor.continue();
        });
      ...
      test('index keyCursor', async () => {
        ...
        await v.db.whenReady();
        v.ans = [];
        v.db.index('TestModel', 'name').keyCursor(null, 'prev', (cursor) => {
          if (!cursor) return;
